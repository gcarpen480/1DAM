...
#El fichero calificaciones.csv contiene las calificaciones de un curso. Durante el curso se realizaron dos exámenes parciales de teoría y un 
#examen de prácticas. Los alumnos que tuvieron menos de 4 en alguno de estos exámenes pudieron repetirlo al final del curso (convocatoria ordinaria). Realiza un programa que haga lo siguiente:
...
with open ("calificaciones.csv","r") as fichero_carretero:
    contenido_carretero = csv.reader(fichero_carretero, delimiter = ",")

    ordenados_carretero = sorted(contenido_carretero,key=lambda x:x[0])
    listado_carretero = list (ordenados_carretero)

    print(listado_carretero[1])
    print(listado_carretero[0])

    for i in range (2,17):
        print(listado_carretero[i])